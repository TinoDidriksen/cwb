...
cqp - The CWB Corpus Query Processor (interactive or slaved-process version)
...
=head1 OPTIONS
    ...
    -e           enable input line editing
    -C           enable ANSI colours (experimental)
    -f filename  execute commands from file (batch mode)
    -p           turn pager off
    -P pager     use program <pager> to display query results
