...
to explain all the details of the CWB data model or how encoding is accomplished. For details on these other issues, 
...
Creates a registry entry (in the file <registry_file>. 
...
(or in names of attributes). This does not apply to the path of the directory that contains the registry file.
...
It is usual to put registry files in a special corpus registry directory, in which case you should specify the full
path of that registry directory for this option. 
