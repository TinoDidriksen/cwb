...
word occuring in a sequence with one directly after the other.
...
A hash table, where each hash contains a particular number of buckets, are used in the scanning
...
buckets are do not need to understand this option. The default number of buckets is 1,000,000.

=item B<-C>        

Enables cleanup of the output data. "Cleanup" means
that any token values that are not "regular" words will be omitted. A regular word
consists only of letters, numbers and hyphens and does not end with a hyphen.f

Cleanup does not apply to the values of "constraint" keys.
...
Adds up frequency values from p-attribute I<attribute>, so that the values each corpus position
can be treated as having a higher frequency than the default 1.

The specified p-attribute must contain
only strings representing numbers, where the number for a given token represents the joint frequency
of the set of items contained on the other p-attributes for that same token.

B<TODO check the above is correct>
...
Activates quiet mode - the progress information normally printed to standard error
is suppressed.
...
be listed one per line, in the form of a start position, followed by a space, followed by
an end position.
