...
in the source corpus correspond to which units in the target corpus.
...
in both the source AND target corpora). If, for instance, sentences are represented as <s> in one corpus
but as <sent> in the other, then they cannot be aligned by this program.
...
sentence in target). B<cwb-align> will only output the 2:2 alignment if the quality score for the 2:2 
alignment exceeds the I<combined> quality scoress of the two separate 1:1 by at least this factor. Otherwise,
it will instead output the two 1:1 alignments.
...
you should experiment with increasing this factor; if the progrma does not output 2:2 alignments when it
ought to, then you shouold experiment with decreasing this factor.
...
pre-alignment with ID matching (identical annotation strings)
...
The configuration flags I<also> specify how much weight should be placed on the corresponding set of features:
...
This means that, by default, the following tyopes of feature are used: (1) length of sentence, with weight 1;
...
=for comment The above appears as: 
"-C:1 -S:50:0.4 -3:3 -4:4"
in the rendered file.

=head1 OUTPUT FORMAT

B<cwb-align>'s output file uses CWB's C<.align> file format. C<.align> files are formatted as follows.
...
a single pair of aligned regions (sometimes called an I<alignment bead>) in the corpus.
