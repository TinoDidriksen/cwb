...
/* set <row>th entry in <fld> column to <value>; automatically allocates and extends columns */

int *table_get_vector(table t, FieldType fld);
/* pointer to int vector representing column <fld> for direct access; note that sortidx is ignored! */

int *table_get_sortidx(table t);
/* pointer to sort index as int vector; returns NULL if table is unsorted */
...
/* may want to change these ops and/or their exact semantics; however, for a smooth migration
