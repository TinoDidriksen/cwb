...
static char *srt_maptable;	/* character mapping for %c and %d flags */
...
SortExternally(void) {		/* uses settings from static srt_* variables */
  ...
  if ((fd = OpenTemporaryFile(temporary_name)) != NULL) {
	...
	if (srt_maptable) {
	  token = p1start;
	  for (l=1 ; l <= plen ; l++) {
	    unsigned char *value = cl_cpos2str(srt_attribute, token);
	    if (value) {
	      int i, p = strlen(value);
	      if (srt_reverse) {
		for (i = p-1 ; i >= 0; i--)
...
static int i2compare(const void *vidx1, const void *vidx2)
{
  ...
  for (pass = (srt_maptable) ? 1 : 2; (pass <= 2) && (comp == 0); pass++) {
	...
	s1 = cl_id2str(srt_attribute, id1);
	s2 = cl_id2str(srt_attribute, id2);
