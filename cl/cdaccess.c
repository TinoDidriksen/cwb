...
char *cdperror_string(int errno)
{
  ...
  case CDA_OK:
    ...
    break;
  case CDA_ENULLATT:
    s = "NULL passed as attribute argument of function";
    break;
  case CDA_EATTTYPE:
    s = "CDA: function called with illegal attribute type";
    break;
  case CDA_EIDORNG:
    s = "CDA: id is out of range";
    break;
  case CDA_EPOSORNG:
    s = "CDA: position is out of range";
    break;
  case CDA_EIDXORNG:
    s = "CDA: index is out of range";
    break;
  case CDA_ENOSTRING:
    s = "CDA: no such string encoded";
    break;
  case CDA_EPATTERN:
    s = "CDA: illegal regular expression/illegal pattern";
    break;
  case CDA_ESTRUC:
    s = "CDA: no structure defined for this position";
    break;
  case CDA_EALIGN:
    s = "CDA: no alignent defined for this position";
    break;
  case CDA_EREMOTE:
    s = "CDA: error during access of remote data";
    break;
  case CDA_ENODATA:
    s = "CDA: can't load and/or create necessary data";
    break;
  case CDA_EARGS:
    s = "CDA: error in arguments of dynamic call";
    break;
  case CDA_ENOMEM:
    s = "CDA: not enough memory";
    break;
  case CDA_EOTHER:
    s = "CDA: unspecified error";
    break;
  case CDA_ENYI:
    s = "CDA: unimplemented feature/not yet implemented";
    break;
  case CDA_EBADREGEX:
    s = "CDA: bad regular expression";
    break;
  default:
    s = "CDA: ILLEGAL ERROR NUMBER";
    break;
  }
  return s;
}
  ...
  *struc_end = 0;
  ...
  if (val != NULL) {
    ...
    *struc_end   = ntohl(*(val + 1));
    ...
    cderrno = CDA_OK;		/* we do not consider this an error */
    return 0;
  }
...
int get_extended_alignment(int *data, int size, int position)   /* XALIGN component */
{
  ...
  */
  ...
  return -1;    /* high < low --> search failed  */
}
  ...
  if (! cl_has_extended_alignment(attribute)) /* subsumes check_arg() */
	...
	return cderrno;
      ...
      alg = get_extended_alignment(align_data->data.data, 
				   ...
				   cpos);
      cderrno = CDA_OK;
      return alg;		/* (alg == -1) is NOT an error */
    }
}
