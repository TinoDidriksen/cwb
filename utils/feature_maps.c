		    ...
		    ) 
{
  ...
  for(i=1;i<nw1;i++) {
    if(r->w2f1[i+1]-r->w2f1[i]!=fcount1[i]-fcount1[i-1]) {
      fprintf(stderr,"ERROR: fcount1[%d]=%d r->w2f1[%d]-r->w2f1[%d]=%d w=``%s''\n",
  ...
  for(i=1;i<nw2;i++) {
    if(r->w2f2[i+1]-r->w2f2[i]!=fcount2[i]-fcount2[i-1]) {
      fprintf(stderr,"ERROR: fcount2[%d]=%d r->w2f2[%d]-r->w2f2[%d]=%d w=``%s''\n",
